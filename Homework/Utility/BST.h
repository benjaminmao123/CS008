...
public:
    ...
    bool search(const T& target, BSTLibrary::tree_node<T>*& found_ptr);
...
inline BST<T>::BST(const T* sorted_list, int size) :
    root(BSTLibrary::tree_from_sorted_list(sorted_list, size))
{
...
inline BST<T>::BST(const BST<T>& copy_me) :
    root(BSTLibrary::tree_copy(copy_me.root))
{
...
{
    BSTLibrary::tree_insert(root, insert_me);
}

template<typename T>
inline void BST<T>::erase(const T& target)
{
    if (!BSTLibrary::tree_erase(root, target))
    {
...
template<typename T>
inline bool BST<T>::search(const T& target, BSTLibrary::tree_node<T>*& found_ptr)
{
    return BSTLibrary::tree_search(root, target, found_ptr);
}
...
{
    BSTLibrary::tree_clear(root);
}

template<typename T>
inline BST<T>& BST<T>::operator+=(const BST<T>& rhs)
{
    BSTLibrary::tree_add(root, rhs.root);
...
{
    BSTLibrary::tree_print(tree.root, 0, outs);
